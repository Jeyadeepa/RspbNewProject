...
namespace RspbNewProject
{
    ...
    public class Reserves : BaseTest
    {
        ...
        //Reserve Overview
        [Test, Category("Smoke Testing")]
        public void ReserveSearchResultsPage()
        {
            try
            {
                IWebElement dropdown = driver.FindElement(By.XPath("//a[normalize-space()='Days out']"));
                ...
                //action.MoveToElement(driver.FindElement(By.XPath("//a[@href='/days-out/reserves/arne']"))).Click().Perform();
                //Thread.Sleep(5000);
                Assert.True(driver.FindElement(By.CssSelector("div span[class='results-title']")).Text.Contains("results"));
                string content = driver.FindElement(By.CssSelector("div span[class='results-title']")).Text;
                Assert.IsTrue(content.Contains("results"));
                Thread.Sleep(5000);
                driver.Close();
            }
            catch (Exception e)
